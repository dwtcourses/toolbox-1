...
func init() {
	rootCmd.AddCommand(asgCompleteCmd)
}


var asgCompleteCmd = &cobra.Command{
	Use: "asg",
	Short: "Interact with Autoscaling group",
	Long: "Perform actions against an AWS Account's Auto scaling groups",
...
func completeInstances(client *autoscaling.AutoScaling, asgName, lifecycleHook string) {
	...
	for _, instance := range instances.AutoScalingInstances {
		if aws.StringValue(instance.AutoScalingGroupName) == asgName {
			log.Infof("Completing lifecycle hook [%v] for [%v]", lifecycleHook, aws.StringValue(instance.InstanceId))
